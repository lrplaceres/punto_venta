...
@router.post("/user", response_model=app.schemas.user.User, status_code=status.HTTP_201_CREATED, tags=["user"])
async def create_user(user: app.schemas.user.UserInDB, token: Annotated[str, Depends(auth.oauth2_scheme)], current_user: Annotated[models.User, Depends(auth.get_current_user)]):
...
@router.get("/user/{id}", response_model=app.schemas.user.User, tags=["user"])
...
async def update_user(id: int, user: app.schemas.user.UserEdit, token: Annotated[str, Depends(auth.oauth2_scheme)], current_user: Annotated[models.User, Depends(auth.get_current_user)]):
...
@router.get("/users", response_model=List[app.schemas.user.UserList], tags=["users"], description="Listado de usuarios")
...
@router.get("/users-propietarios", response_model=List[app.schemas.user.UserList], tags=["users"], description="Listado de usuarios con rol propietario")
...
async def update_user(user: app.schemas.user.UserCambiaPassword, token: Annotated[str, Depends(auth.oauth2_scheme)], current_user: Annotated[models.User, Depends(auth.get_current_user)]):
...
async def update_user(id: int, user: app.schemas.user.UserCambiaPasswordAdmin, token: Annotated[str, Depends(auth.oauth2_scheme)], current_user: Annotated[models.User, Depends(auth.get_current_user)]):
